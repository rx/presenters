module Voom::Presenters::WebClient::Helpers
  module Headers
    ...
    def plugin_headers(pom)
      Voom::Presenters::WebClient::PluginHeaders.new(pom: pom, render: method(:render)).render
    end

    def custom_css(path, host=nil)
      root = Voom::Presenters::Settings.config.presenters.root
      Voom::Presenters::WebClient::CustomCss.new(path, root: root, host: host).render
    end

    def custom_js
      custom_js_path = Voom::Presenters::Settings.config.presenters.web_client.custom_js
