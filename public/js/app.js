...
$(document).ready(function () {
    $('a').click(function () {
        ...
        if(hash){
            var startParams = hash.indexOf('(');
            var endParams = hash.lastIndexOf(')');
            var command = hash.substring(1, startParams);
            var params = hash.substring(startParams+1, endParams);
            params = params.split(',');
            Voom.Commands[command].apply(this, params);
  }
        return hash ? false : true; //stop default link behaviour
    });
});


$(function () {
    Voom.Commands.modal = function (id) {
        $('#'+id).modal('show');
    };
    $.anchorHandler.add(/\#modal/, Voom.Commands.modal);
});
$(function () {
    Voom.Commands.section = function (url, section_id) {
        ...
        jqXHR = $.ajax(url)
          ...
          .fail(function(xhr, status, errorThrown) {
              section.html('<h2>Oops, ther server was none too happy with that last request.</h2>Status Code: ' + xhr.status + '<br\>' + errorThrown);
          });
    };
});
...
/* Handles anchor links in the document by firing a javascript function.
 * Sample:
 * $.anchorHandler
...
var evaluate = function(template){
    eval('`' + template + '`')
};
