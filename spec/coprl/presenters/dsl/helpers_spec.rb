...
describe 'helpers' do
  let(:helpers1_pom) {Coprl::Presenters::App[:helpers1].call.expand(router: nil, context: context)}
  let(:helpers2_pom) {Coprl::Presenters::App[:helpers2].call.expand(router: nil, context: context)}
  let(:context) {}
  after do
    reset_presenters!
  end
  ...
  describe 'are accessable in nested components' do
    ...
    let(:pom) {Coprl::Presenters::App[:helpers_in_components].call.expand(router: nil, context: context)}
    it 'reaches baz' do
